-- N-dimensional grid implemented with a flattened Array
    ...
    Maybe.map Tuple.first <|
        if List.length indices /= List.length grid.shape then
                ...
                List.foldl 
                    ...
                    (Just (0, 1))
                    zipped
getElement : List Int -> Maybe (Grid a) -> Maybe a
getElement indices grid = 
    Maybe.andThen
        (\gridd ->
            let
...
fold : (List Int -> a -> b -> b) -> b -> Grid a -> b
fold f init grid =
